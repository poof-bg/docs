...
const result = await poof.remove(imageBuffer);
...
sequenceDiagram
    participant You
    participant Poof API
    participant AI Engine
    
    You->>Poof API: POST /remove with image
    Poof API->>AI Engine: Process image
    AI Engine->>Poof API: Return processed image
    Poof API->>You: Image with background removed
```
