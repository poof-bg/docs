...
<Note>
The type hints in `poofbg` 0.1.0 lag behind the API: they still list old `size` values and omit `alpha` for `channels`. Values are passed through to the API unchanged, so use the values above and ignore the hint. `padding`, `width`, `height` and `fit` are not exposed by the SDK yet; call the [REST API](/api-reference/remove-background) directly if you need them.
</Note>
