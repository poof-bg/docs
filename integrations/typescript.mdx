...
<Note>
The `ImageSize` and `Channels` types in `@poof-bg/js` 1.0.0 lag behind the API: they still list old `size` values and omit `'alpha'`. Values are passed through to the API unchanged, so use the values above (cast if TypeScript complains). `padding`, `width`, `height` and `fit` are not exposed by the SDK yet; call the [REST API](/api-reference/remove-background) directly if you need them.
</Note>
