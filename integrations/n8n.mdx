---
title: n8n
description: "Integrate Poof with n8n for powerful open-source workflow automation"
---
...
| **Size** | Full, Preview, Medium, HD | Output dimensions |
| **Channels** | RGBA, RGB, Alpha | Transparent, opaque, or grayscale alpha mask only |
| **Background Color** | Hex color | Fill color when RGB |
| **Crop to Subject** | Boolean | Remove empty space |

<Note>
If your installed version of the node shows different labels, refer to the API values in the [Remove Background reference](/api-reference/remove-background): `size` is `full`, `preview`, `medium` or `hd`; `channels` is `rgba`, `rgb` or `alpha`.
</Note>
...
- `402` — Out of credits
- `429` — Rate limited (per-minute limit for your plan), add a Wait node

## Rate Limiting

Each plan has a requests-per-minute limit (Free 4, Basic 8, Pro 12, Mega 40, Ultra 100, Giga 200). Keep the batch size at or below your plan's limit and wait 60 seconds between batches. For example, on the Free plan:

```
Split In Batches (batch size: 4) → Poof → Wait (60 seconds)
```

See [Rate Limit Exceeded](/errors/rate-limit-exceeded) for the full table.
...
<Tip>
**Use Preview size for testing** — It's faster while you're building your workflow. Every successful request costs 1 credit regardless of size.
</Tip>
...
<Tip>
**Batch your operations** — Process images in groups no larger than your plan's per-minute limit, with a 60-second wait between groups, to avoid rate limits.
</Tip>
