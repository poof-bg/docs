...
<Tip>
**Smallest possible change: the hostname and the key.** Poof also serves the remove.bg path, so `POST https://api.poof.bg/v1.0/removebg` behaves exactly like `POST https://api.poof.bg/v1/remove`. It is a route alias, not an HTTP redirect, so POST bodies and headers are untouched. A remove.bg client that only swaps `api.remove.bg` for `api.poof.bg` and sets a Poof key in `X-Api-Key` keeps working. Use `/v1/remove` in new code; it is the canonical path.
</Tip>
