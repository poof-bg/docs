...
```python Python
result = client.remove("product.jpg", crop=True, width=500, height=500)
result.save("product-500.png")
```

```typescript TypeScript
const result = await poof.remove(image, { crop: true, width: 500, height: 500 });
```
...
```python Python
result = client.remove("icon.png", width=1000, height=1000, fit="scale-down")
```

```typescript TypeScript
const result = await poof.remove(image, { width: 1000, height: 1000, fit: 'scale-down' });
```
