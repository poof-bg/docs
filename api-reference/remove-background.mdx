...
```python Python
# padding is not exposed by the SDK yet; call the REST API directly
import httpx
...
with open("product.jpg", "rb") as f:
    r = httpx.post(
        ...
        files={"image_file": f},
        data={"crop": "true", "padding": "10%,7.5%"},
    )
r.raise_for_status()
open("product-cropped.png", "wb").write(r.content)
```

```typescript TypeScript
// padding is not exposed by the SDK yet; call the REST API directly
const form = new FormData();
form.append('image_file', new Blob([await fs.readFile('product.jpg')]), 'product.jpg');
form.append('crop', 'true');
form.append('padding', '10%,7.5%');
...
await fs.writeFile('product-cropped.png', Buffer.from(await res.arrayBuffer()));
```
